     ...
     var append = '  <form class="frm_edititem">\
						...
						<div class="form-group">\
							...
							<select name="e_category" class="form-control" required>\
								...
								<option>Audio-Visual Equipment</option>\
...
// if (roomChangeNeeded) {
// append = append.replace('class="form-control hide"', 'class="form-control" required="required"');
// }
...
// tbl_borrower.on('click', 'button', function(e){
// 	e.preventDefault();
...
// 		if(data != 0){
// 			toastr.success('Successully returned items');
// 			tbl_borrower.ajax.reload(null,false);
// 			tbl_return.ajax.reload(null,false);
// 		}else{
// 			toastr.error('Failed');
// 		}
...
table_faculty.on('click', 'a.edit-member', function(e){
	e.preventDefault();
	$('.divedit-member').toggle(effect, options, duration);
	var tr = table_faculty.row( $(this).parents('tr') ).data();
	var yrs = tr[5].split('-');
	...
	$('.frm_student_sign').submit(function(e){
		e.preventDefault();
		var frmdata = $(this).serialize()+'&key=edit_faculty';
		...
		.done(function(data){
			console.log(data);
			if(data == 1){
				table_faculty.ajax.reload(null,false);
				toastr.success('Successully updated');
				$('.divedit-member').toggle(effect, options, duration);
			}else{
				toastr.error('Failed to update');
...
table_faculty.on('click', '.deactivate-member', function(e){
	e.preventDefault();
	var tr = table_faculty.row( $(this).parents('tr') ).data();
	id = tr[10];
...
table_faculty.on('click', '.activate-member', function(e){
	e.preventDefault();
	var tr = table_faculty.row( $(this).parents('tr') ).data();
	id = tr[10];
