...
// }
		...
		public function add_equipment()
		{
			...
			$e_assigned = !empty($_POST['e_assigned']) ? $_POST['e_assigned'] : 0;
		...
		public function add_newfaculty($f_id, $f_fname, $f_lname, $f_gender, $f_contact, $f_department, $type)
		{
			...
			if ($sql_count <= 0) {
				...
				// If the insert was successful, insert into the history_logs table
				if ($insert_count > 0) {
					// Removed history_logs insert as we no longer accept parameters for it
					echo "1";
				} else {
					echo "0";
				}
	...
	switch ($key){
		...
		$e_assigned = isset($_POST['e_assigned']) ? trim($_POST['e_assigned']) : null; 
