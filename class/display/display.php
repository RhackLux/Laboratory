<?php
	...
	*/
	class display
	{
		...
		// {
		// 	global $conn; 
		// 	$sql = $conn->prepare("SELECT * FROM member WHERE = ");
		// 	$sql->execute();
		// 	$count = $sql->rowCount();
		// 	$fetch = $sql->fetchAll();
		// 	if($count > 0){
...
{
    global $conn; 
    $sql = $conn->prepare("SELECT * FROM member WHERE m_type = 'Student'");
    $sql->execute();
    $count = $sql->rowCount();
    $fetch = $sql->fetchAll();
    if($count > 0){
...
{
	global $conn; 
	$sql = $conn->prepare("SELECT * FROM member WHERE m_type = 'Faculty'");
	$sql->execute();
	$count = $sql->rowCount();
	$fetch = $sql->fetchAll();
	if($count > 0){
		...
		// 			$data['data'][] = array('<img src="'.$photo.'" style="height=75px;width:75px;object-fit:cover" class="img-responsive" />',$value['i_model'],$value['i_category'],$value['i_brand'],$value['i_description'],$value['items_stock'],$str_status,$button);
		// 		}
		// 		echo json_encode($data);
		// 	}else{
		// 		$data['data'] = array();
			...
			public function display_equipment()
			{
				...
				if($row > 0){
					foreach ($fetch as $key => $value) {
						...
						$photo = ($value['i_photo'] == "") ? "../assets/noimagefound.jpg" : "../uploads/" . $value['i_photo'];
						$data['data'][] = array(
							'<img src="'.$photo.'" style="height=75px;width:75px;object-fit:cover" class="img-responsive" />',  // Image
							$value['i_model'],    // Model
							$value['i_category'], // Category
							$value['i_brand'],    // Brand
							$value['i_description'], // Description
							$value['items_stock'], // Quantity Left
							$str_status,          // Status
							$button               // Action
						);
					}
		...
		// 					</button>";
		// 		$date = date(format: 'F d,Y H:i:s A', timestamp: strtotime($value['date_borrow']));
		// 			$data['data'][] = array($value['m_fname'].' '.$value['m_lname'],$date,$value['item_borrow'],ucwords($value['rm_name']),$button);
		// 		}
		...
		public function display_borrow()
		{
			...
			if($count > 0){
				foreach ($fetch as $key => $value) {
					...
					$date = date('F d,Y H:i:s A', strtotime($value['date_borrow'])); // Corrected date formatting
					$data['data'][] = array(
						$value['m_fname'].' '.$value['m_lname'], // Borrower Name
						$date,                                   // Borrowed Date
						$value['item_borrow'],                   // Items Borrowed
						ucwords($value['rm_name']),              // Room
						$button                                   // Action Button
					);
				}
				echo json_encode($data); // Removed 'value:' to match JSON format
			} else {
				$data['data'] = array();
				echo json_encode($data);
			}
		...
		public function display_borrower()
		{
			...
			// Add conditions for month and year if they are set
			$conditions = [];
			if (isset($_POST['month']) && $_POST['month'] != "") {
				$month = intval($_POST['month']);
				$conditions[] = "MONTH(borrow.date_borrow) = $month";
			}
			if (isset($_POST['year']) && $_POST['year'] != "") {
				$year = intval($_POST['year']);
				$conditions[] = "YEAR(borrow.date_borrow) = $year";
			}
			...
			$sql .= " GROUP BY borrow.borrowcode";
		...
		public function Alternate_tbl()
		{
			...
			if ($count > 0) {
				foreach ($fetch as $key => $value) {
					$date = date('F d,Y H:i:s A', strtotime($value['date_borrow']));
					// Remove the button and pass other relevant data
					$data['data'][] = array($value['m_fname'].' '.$value['m_lname'], $date, $value['item_borrow'], ucwords($value['rm_name']));
				}
				echo json_encode($data);
			} else {
				$data['data'] = array();
	...
	switch ($key) {
		...
		case 'display_facultyselect';
		$display->display_facultyselect();
		break;
		...
		case 'display_borrower';
		$display->display_borrower();
		break;
